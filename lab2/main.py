...
# -*- coding: utf-8 -*-
"""
Программа управления парковкой (MDP) с использованием value iteration.

Моделируется процесс принятия решений о цене парковки как марковский процесс
принятия решений (MDP). Оператор может устанавливать одну из трёх цен (низкую,
среднюю, высокую) в зависимости от текущего количества свободных мест. Цель —
максимизировать суммарный дисконтированный доход.

Основные элементы модели:
    • Состояния (s) — количество свободных мест на парковке (0 … C).
    • Действия (a) — выбор цены: низкая (low), средняя (mid), высокая (high).
    • Вероятности переходов — определяются двумя процессами:
        1) Приезд автомобиля с вероятностью p_arrival(a) (зависит от цены).
        2) Отъезды автомобилей: каждая из занятых машин уезжает с вероятностью q
           независимо от других (биномиальное распределение).
      Порядок событий в каждом шаге: сначала приезд (если есть свободное место),
      затем отъезды.
    • Награда (reward) — доход, полученный от приехавшего автомобиля (цена),
      начисляется только в момент приезда. Если приезда нет, награда равна 0.
    • Коэффициент дисконтирования γ (0 ≤ γ ≤ 1) — определяет важность будущих
      доходов. Чем ближе γ к 1, тем более дальновидным является агент.
    • Вероятность отъезда q (0 ≤ q ≤ 1) — вероятность того, что одна занятая
      машина покинет парковку в течение шага.

Алгоритм value iteration:
    Value iteration итеративно обновляет функцию ценности V(s) — ожидаемый
    суммарный дисконтированный доход из состояния s при оптимальной политике.
    На каждом шаге для всех состояний вычисляются Q-значения для каждого действия:
        Q(s,a) = R(s,a) + γ * Σ_{s'} P(s'|s,a) * V(s')
    где R(s,a) — ожидаемая награда в состоянии s при действии a,
    P(s'|s,a) — вероятности переходов.
    Затем V(s) обновляется как max_a Q(s,a).
    Итерации продолжаются, пока максимальное изменение V(s) не станет меньше ε.
    После сходимости извлекается оптимальная политика π(s) = argmax_a Q(s,a).

    Сложность алгоритма:
        O(C * |A| * (C+1)) за итерацию, где C — вместимость, |A| = 3.

Параметры, задаваемые пользователем:
    • C — вместимость парковки (целое число ≥ 1).
    • γ — коэффициент дисконтирования (0 … 1).
    • q — вероятность отъезда одной машины (0 … 1).
    • Цены (low_price, mid_price, high_price) — неотрицательные числа.
    • Вероятности приезда (p_low, p_mid, p_high) — для каждой цены (0 … 1).
    • s_start — начальное количество свободных мест (0 … C).
    • ε — точность сходимости value iteration (малое положительное число).

Выходные данные:
    После расчёта программа выводит:
    1) Функцию ценности V(s) для всех состояний s = 0 … C.
    2) Оптимальную политику π(s) — какую цену устанавливать при каждом
       количестве свободных мест. Для состояния s=0 (парковка полностью занята)
       политика может быть помечена как «любая», поскольку приезд невозможен и
       все действия приводят к одинаковым Q-значениям.
    3) Ценность начального состояния V(s_start).

Интерактивное меню:
    После ввода параметров программа переходит в главное меню, где можно:
        1) Вычислить — запустить value iteration с текущими параметрами.
        2) Внести изменения — отредактировать любой параметр (C, γ, q, цены,
           вероятности, начальное состояние, ε). При изменении C начальное
           состояние автоматически корректируется, если оно выходит за пределы.
        3) Help — показать эту справку.
    Для выхода из программы используйте Ctrl+C в любой момент.

Примечания:
    • Вычисления используют точные вероятности переходов через биномиальное
      распределение (функция math.comb).
    • Для форматирования вывода используется библиотека rich (таблицы, панели).
    • Программа написана на Python 3 и требует установки rich (pip install rich).

Вариант 4: Управление парковкой (MDP).
"""
